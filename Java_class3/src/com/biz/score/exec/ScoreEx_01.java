...
public class ScoreEx_01 {
	...
	public static void main(String[] args) {
		...
		/*
		 * ScoreServiceV1에 선언되어 있는 ScoreVO[] scores는 아직 선언만되고 초기화 등이
		 * 전혀 이루어지지 않은 상태
		 * 
		 * 때문에 scores 인스턴스 배열이 초기화되거나 값이 저장되지 않으면
		...
		/*
		 * ScoreServiceV1에서는 scores배열을 주입하는 method setScore()라고 별도로
		 * 작성해 두었는데 사용하기 위해 작성을 하는 과정에서 setScore()를 먼저
		 * 호출해서 scores를 전달해야 하는 것을 자주 잊게 되더라
		 * 
		 * 그리하여 NullPointException이 발생하는 경우가 많더라
		 * 
		 * 그래서 ScoresServiceV2에서는 성적처리를 하는 과정에서 반드시 호출할 수 밖에
		 * 없는 메서드들 중에 호출되는 순서로 봤을 때 항상 제일 먼저호출할 것으로
