...
public class Grade_09 {
	...
	for(int i = 1 ; i <= 2; i++) {
		/*
		 * for()가 반복되면서 사용되는 변수 i를 다른 변수에 부착하여 아래와 같이 사용하면 num_1 = 90이라는 코드로 바뀌어
		 * 사용할 수 있을 것 같은데 문법상 이코드는 num_i라는 변수를 찾는 코드로 전환되어 sysntax오류를 발생한다.
		 * 이러한 코드는 사용이 불가능하다.
		 * num_i = 90;
		 */
	}
	...
	// 배열 100개에 담겨있는 값을 intSum변수에 더하여 총점 구하기
	int intSum = 0;
	for(int i = 0 ; i < 100 ; i++) {
