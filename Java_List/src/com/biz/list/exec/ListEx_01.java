...
public class ListEx_01 {
	...
	public static void main(String[] args) {
		...
		// 			 new : rnd 인스턴스를 초기화 하겠다라는 키워드
		//				 생성자(Contructor) method
		Random rnd = new Random();
		// Random 클래스를 상요해서 rnd 인스턴스를 선언하고
		// Random() 생성자를 호출하여 rnd 인스턴스를 초기화(새로만들기)하라

		/*
		 * 다른 클래스와 색다르게 선언하는 클래스와 초기화하는 생성자의
		 * 이름이 다른 형태를 갖는다.
		 * 
		 ...
		 * Integer형 리스트라고 부른다.
		 * 
		 * 자바에서 기본 배열형 데이터를 사용하는데 어려움이 있어서 JDK에서 제공해주는
		 * 도구를 사용하여 많은 개수의 데이터르 관리하기 위한 방법을 제공한다.
		 * 
		 */
		List<Integer> intList = new ArrayList<Integer>();
		// 리스트를 처음 만들면 size() 값이 0인 데이터가 아무것도 없는
		// 리스트가 만들어진다.
		...
		System.out.println("100개의 임의 수 합 : " + sum);
