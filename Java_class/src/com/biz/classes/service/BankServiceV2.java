 ...
 * method를 호출해서 사용해야 한다.
 */
public class BankServiceV2 {

	/*
	 * method()
	 * 어떤 코드들의 묶음을 만들어 두고 사용할 때 한, 두줄의 명령문만으로
	 * 기능을 수행하고자 할 때 작성하는 코드
	 */
	...
	/*
	 * BankVO클래스로 생성한 인스턴스에 담긴 데이터들을 매개변수로 받아서
	 * 무언가 처리를 하는 method
	 */
	...
	/*
	 * BankVO클래스로 생성한 인스턴스 "배열에" 담긴 데이터들을 매개변수로 받아서
	 * 무언가 처리하는 method
	 */
	public void bankList(BankVO[] bankList) {
		...
		for(int i = 0; i < bankList.length ; i++) {
			...
			System.out.printf("%s\t%s\t%d\t%d\t%d\n", bankList[i].getStrnum(), bankList[i].getStrdate(), bankList[i].getIntinput(),
					bankList[i].getIntoutput(), sum);
		}
	}
}
