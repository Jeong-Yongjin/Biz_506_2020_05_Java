 ...
 * 한 클래스내에서 중복정의하여 사용할 수 있다.
 * 
 * 
 * java 이전의 언어에서는 클래스를 모듈이라는 이름으로 불렀는데
 * 1개의 모듈, 1개의 파일내에서는 같은 이름의 method(함수)를 정의 할 수 없었다.
 * 
 * java에서는 method의 매개변수 개수가 다르면 같은 이름의 method를 사용할 수 있다.
 * 
 * 호출하는 곳에서 매개변수 전달을 다르게 수행하면 자동으로
 * method가 연결되어 코드가 수행 된다.
 * 
 * 또한, method의 매개변수 개수가 같은 경우라도 매개변수의 type이 다르면
 * 이름이 같은 method를 정의할 수 있다.
 */
